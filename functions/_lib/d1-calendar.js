...
async function ensureCalendarSchemaUncached(db) {
  ...
  `).run();
  await db.prepare("CREATE INDEX IF NOT EXISTS idx_roster_day_coworkers_doctor_date ON roster_day_coworkers (doctor_key, date)").run();
  await db.prepare("CREATE INDEX IF NOT EXISTS idx_roster_day_coworkers_date_source ON roster_day_coworkers (date, source_type)").run();
  await db.prepare(`
  ...
  `).run();
  await db.prepare("CREATE INDEX IF NOT EXISTS idx_roster_term_overlap_doctor_date ON roster_term_overlap_doctors (doctor_key, date)").run();
  await db.prepare("CREATE INDEX IF NOT EXISTS idx_roster_term_overlap_date_source ON roster_term_overlap_doctors (date, source_type)").run();
  await db.prepare(`
...
export async function queryCoworkerEventsFromInsightCache(db, options = {}) {
  if (!db?.prepare) return [];
  await ensureCalendarSchema(db);
  const start = String(options.startDate || options.date || "0000-01-01");
  const end = String(options.endDate || options.date || "9999-12-31");
  ...
  if (overlapKeys.length) {
    const rows = await db.prepare(`
      SELECT DISTINCT
        ...
        p.display_name,
        p.source_type
      FROM roster_day_coworkers AS mine
      INNER JOIN roster_day_coworkers AS p
        ON p.date = mine.date
       AND p.source_type = mine.source_type
      INNER JOIN roster_events AS ev ON ev.id = p.event_id
      WHERE mine.doctor_key IN (${overlapKeys.map(() => "?").join(", ")})
  ...
  const rows = await db.prepare(`
    SELECT DISTINCT
      ...
      p.display_name,
      p.source_type
    FROM roster_day_coworkers AS p
    INNER JOIN roster_events AS ev ON ev.id = p.event_id
    WHERE p.date >= ?
      AND p.date <= ?
      ${sourceSql}
...
export async function queryOverlapDoctorsFromInsightCache(db, options = {}) {
  if (!db?.prepare) return [];
  await ensureCalendarSchema(db);
  const start = String(options.startDate || options.date || "0000-01-01");
  const end = String(options.endDate || options.date || "9999-12-31");
  ...
  const excludeKeys = [...new Set((options.excludeDoctorKeys || []).filter(Boolean))];
  if (!overlapKeys.length) return [];
  const sourceSql = sourceTypes.length ? `AND source_type IN (${sourceTypes.map(() => "?").join(", ")})` : "";
  const excludeDoctorSql = excludeKeys.length ? `AND overlap_doctor_key NOT IN (${excludeKeys.map(() => "?").join(", ")})` : "";
  const rows = await db.prepare(`
    SELECT DISTINCT
      overlap_doctor_key AS doctor_key,
      overlap_display_name AS display_name,
      source_type
    FROM roster_term_overlap_doctors
    WHERE doctor_key IN (${overlapKeys.map(() => "?").join(", ")})
      AND date >= ?
      AND date <= ?
      ${sourceSql}
      ${excludeDoctorSql}
    ORDER BY overlap_display_name, source_type
  `).bind(...overlapKeys, start, end, ...sourceTypes, ...excludeKeys).all();
  return (rows.results || []).map((row) => ({
    doctorKey: row.doctor_key,
    displayName: row.display_name,
    sourceType: row.source_type,
...
export async function queryRosterInsightCacheRangesForFile(db, fileId) {
  if (!db?.prepare || !fileId) return [];
  await ensureCalendarSchema(db);
  const rows = await db.prepare(`
    SELECT
  ...
  `).bind(fileId).all();
  return insightCacheRangesFromRows(rows.results || []);
}

export async function queryRosterInsightCacheRangesForFiles(db, fileIds = []) {
  if (!db?.prepare || !fileIds?.length) return [];
  await ensureCalendarSchema(db);
...
export async function rebuildRosterInsightCachesForRanges(db, ranges = []) {
  if (!db?.prepare) return { ranges: 0 };
  await ensureCalendarSchema(db);
  const normalized = normalizeInsightCacheRanges(ranges);
  let rebuilt = 0;
  for (const range of normalized) {
    await rebuildRosterInsightCacheForRange(db, range);
    rebuilt += 1;
  }
  return { ranges: rebuilt };
}

export async function rebuildRosterInsightCachesForActiveFiles(db, options = {}) {
  if (!db?.prepare) return { files: 0, ranges: 0, done: true, nextOffset: null };
  await ensureCalendarSchema(db);
  const offset = Math.max(0, Number.parseInt(options.offset ?? 0, 10) || 0);
  const limit = Math.max(1, Math.min(Number.parseInt(options.limit ?? 10, 10) || 10, 25));
  const rows = await db.prepare(`
  ...
  const ranges = [];
  for (const activeFileId of batchFileIds) {
    ranges.push(...await queryRosterInsightCacheRangesForFile(db, activeFileId));
    await rebuildDailyPresenceForFile(db, activeFileId);
  }
  const rebuilt = await rebuildRosterInsightCachesForRanges(db, ranges);
  const done = fileIds.length <= limit;
  return {
    files: batchFileIds.length,
...
function insightCacheRangesFromRows(rows = []) {
  const ranges = [];
  for (const row of rows) {
    ...
    if (!sourceType || !startDate || !endDate) continue;
    for (const termKey of insightTermKeysForRange(startDate, endDate)) {
      ranges.push({ termKey, sourceType });
    }
  }
  return normalizeInsightCacheRanges(ranges);
}

function normalizeInsightCacheRanges(ranges = []) {
  const seen = new Set();
  const normalized = [];
  for (const range of Array.isArray(ranges) ? ranges : []) {
    const termKey = String(range?.termKey || "").trim();
    const sourceType = normalizeSourceType(range?.sourceType || range?.source_type || "");
    if (!termKey || !sourceType) continue;
    const marker = `${termKey}|${sourceType}`;
    if (seen.has(marker)) continue;
    seen.add(marker);
    normalized.push({ termKey, sourceType });
  }
  return normalized.sort((left, right) => left.termKey.localeCompare(right.termKey) || left.sourceType.localeCompare(right.sourceType));
}

async function rebuildRosterInsightCacheForRange(db, range) {
  const term = termBoundsFromInsightKey(range.termKey);
  if (!term) return;
  const sourceType = normalizeSourceType(range.sourceType);
  if (!sourceType) return;
  const updatedAt = new Date().toISOString();
  await db.prepare("DELETE FROM roster_day_coworkers WHERE term_key = ? AND source_type = ?").bind(range.termKey, sourceType).run();
  await db.prepare("DELETE FROM roster_term_overlap_doctors WHERE term_key = ? AND source_type = ?").bind(range.termKey, sourceType).run();
  const dayRows = await db.prepare(`
    ...
    FROM roster_daily_presence AS p
    INNER JOIN roster_events AS ev ON ev.id = p.event_id
    INNER JOIN roster_files AS files ON files.id = ev.file_id
    WHERE files.active = 1
      AND p.source_type = ?
      AND p.date >= ?
      AND p.date <= ?
  `).bind(sourceType, term.startDate, term.endDate).all();
  await insertRosterDayCoworkerRows(db, (dayRows.results || []).map((row) => [
    range.termKey,
    row.date,
    row.source_type,
    row.doctor_key,
  ...
  const overlapRows = await db.prepare(`
    SELECT DISTINCT
      mine.date AS date,
      mine.source_type AS source_type,
      mine.doctor_key AS doctor_key,
      mine.display_name AS display_name,
      other.doctor_key AS overlap_doctor_key,
      other.display_name AS overlap_display_name
    FROM roster_daily_presence AS mine
    INNER JOIN roster_daily_presence AS other
      ON other.date = mine.date
     AND other.source_type = mine.source_type
     AND other.doctor_key <> mine.doctor_key
    INNER JOIN roster_events AS mine_event ON mine_event.id = mine.event_id
    INNER JOIN roster_files AS mine_file ON mine_file.id = mine_event.file_id
    INNER JOIN roster_events AS other_event ON other_event.id = other.event_id
    INNER JOIN roster_files AS other_file ON other_file.id = other_event.file_id
    WHERE mine_file.active = 1
      AND other_file.active = 1
      AND mine.source_type = ?
      AND mine.date >= ?
      AND mine.date <= ?
  `).bind(sourceType, term.startDate, term.endDate).all();
  await insertRosterTermOverlapRows(db, (overlapRows.results || []).map((row) => [
    range.termKey,
    row.date,
    row.source_type,
    row.doctor_key,
    row.display_name,
    row.overlap_doctor_key,
    row.overlap_display_name,
    updatedAt,
  ]));
}

async function insertRosterDayCoworkerRows(db, rows) {
  for (const chunk of chunkRows(rows, 100)) {
    if (!chunk.length) continue;
    await db.prepare(`
      INSERT OR REPLACE INTO roster_day_coworkers (term_key, date, source_type, doctor_key, display_name, event_id, updated_at)
      VALUES ${chunk.map(() => "(?, ?, ?, ?, ?, ?, ?)").join(", ")}
    `).bind(...chunk.flat()).run();
  }
}

async function insertRosterTermOverlapRows(db, rows) {
  for (const chunk of chunkRows(rows, 100)) {
    if (!chunk.length) continue;
    await db.prepare(`
      INSERT OR REPLACE INTO roster_term_overlap_doctors (
        term_key, date, source_type, doctor_key, display_name, overlap_doctor_key, overlap_display_name, updated_at
      )
      VALUES ${chunk.map(() => "(?, ?, ?, ?, ?, ?, ?, ?)").join(", ")}
    `).bind(...chunk.flat()).run();
  }
...
function insightTermKeysForRange(startDate, endDate) {
  const keys = [];
  let term = australianTermForDateKey(startDate);
  const finalDate = String(endDate || startDate || "").slice(0, 10);
  while (term && term.startDate <= finalDate) {
    keys.push(term.termKey);
    term = nextAustralianTermBounds(term);
  }
  return keys;
}

function termBoundsFromInsightKey(termKey) {
  const match = String(termKey || "").match(/^(\d{4})-T([1-4])$/);
  if (!match) return null;
  return buildAustralianTermBounds(Number(match[1]), Number(match[2]));
}

function australianTermForDateKey(dateKey) {
  const date = parseDateKeyUtc(dateKey);
  const year = date.getUTCFullYear();
  const candidates = [
    buildAustralianTermBounds(year, 1),
    buildAustralianTermBounds(year, 2),
    buildAustralianTermBounds(year, 3),
    buildAustralianTermBounds(year, 4),
    buildAustralianTermBounds(year - 1, 4),
  ];
  return candidates.find((term) => dateKey >= term.startDate && dateKey < term.nextStartDate) || buildAustralianTermBounds(year, 1);
}

function nextAustralianTermBounds(term) {
  const nextTermNumber = term.termNumber === 4 ? 1 : term.termNumber + 1;
  const nextYear = term.termNumber === 4 ? term.year + 1 : term.year;
  return buildAustralianTermBounds(nextYear, nextTermNumber);
}

function buildAustralianTermBounds(year, termNumber) {
  const start = firstMondayOfMonthUtc(year, [1, 4, 7, 10][Math.max(0, Math.min(3, termNumber - 1))]);
  const nextStart = addUtcDays(start, 91);
  return {
    year,
    termNumber,
    termKey: `${year}-T${termNumber}`,
    startDate: formatUtcDateKey(start),
    endDate: formatUtcDateKey(addUtcDays(nextStart, -1)),
    nextStartDate: formatUtcDateKey(nextStart),
  };
}

function firstMondayOfMonthUtc(year, monthIndex) {
...
function parseDateKeyUtc(dateKey) {
  const [year, month, day] = String(dateKey || "").slice(0, 10).split("-").map((part) => Number(part));
