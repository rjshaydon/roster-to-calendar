...
const failedResponse = await handleDerivedRoster({
  ...
  env: { ROSTER_AUTOMATION_TOKEN: token, ROSTER_AUTOMATION_WRITES_ENABLED: "true", ROSTER_AUTOMATION_QUEUE_ENABLED: "true", ROSTER_AUTOMATION_SOURCE_ALLOWLIST: "future-source", ROSTER_DB: failedDb },
});
assert.equal(failedResponse.status, 200, "an explicitly allowed queued source must accept terminal failure reporting");
...
const processingResponse = await handleDerivedRoster({
  ...
  env: { ROSTER_AUTOMATION_TOKEN: token, ROSTER_AUTOMATION_WRITES_ENABLED: "true", ROSTER_AUTOMATION_QUEUE_ENABLED: "true", ROSTER_AUTOMATION_SOURCE_ALLOWLIST: "future-source", ROSTER_DB: processingDb },
});
