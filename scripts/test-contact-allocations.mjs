...
assert.equal(contactExtractHasExpired("2026-08-24", new Date("2026-08-25T06:59:00Z")), false, "retain Night phone JSON until 17:00 Melbourne the following day during testing");
assert.equal(contactExtractHasExpired("2026-08-24", new Date("2026-08-25T07:00:00Z")), true, "expire Night phone JSON at 17:00 Melbourne the following day during testing");
...
assert.deepEqual(contactsAfterShiftChange(shiftChangeContacts, { date: "2026-08-25", now: new Date("2026-08-25T06:59:00Z") }).map((entry) => entry.shift), ["AM", "PM", "Night"],
  "the previous Night contacts should remain until 17:00 during testing");
assert.deepEqual(contactsAfterShiftChange(shiftChangeContacts, { date: "2026-08-25", now: new Date("2026-08-25T07:00:00Z") }).map((entry) => entry.shift), ["AM", "PM"],
  "Night contacts should clear at 17:00 during testing");
assert.equal(shouldCarryPreviousNightContacts("2026-08-24", "2026-08-25", new Date("2026-08-25T06:59:00Z")), true,
  "the previous extract should remain usable for Night contacts until 17:00 during testing");
assert.equal(shouldCarryPreviousNightContacts("2026-08-24", "2026-08-25", new Date("2026-08-25T07:00:00Z")), false,
  "the previous extract should stop being usable at 17:00 during testing");
assert.equal(shouldUseCurrentExtractForPreviousNight("2026-08-25", "2026-08-24", new Date("2026-08-25T06:59:00Z")), true,
  "a post-handover extract should expose its Night rows on the Night shift's start date during testing");
assert.equal(shouldUseCurrentExtractForPreviousNight("2026-08-25", "2026-08-24", new Date("2026-08-25T07:00:00Z")), false,
  "a post-handover extract should stop exposing yesterday's Night rows at 17:00 during testing");
assert.deepEqual(contactsAfterShiftChange(shiftChangeContacts, { date: "2026-08-25", now: new Date("2026-08-25T00:30:00Z") }).map((entry) => entry.shift), ["AM", "Night"],
  "at 10:30 Melbourne, the previous Night contacts should remain visible during testing");
assert.deepEqual(contactsAfterShiftChange(shiftChangeContacts, { date: "2026-08-25", now: new Date("2026-08-25T05:00:00Z") }).map((entry) => entry.shift), ["AM", "PM", "Night"],
  "after 15:00 Melbourne, PM and retained Night contacts should both be available during testing");
