...
assert.match(appSource, /function facilityOverviewEffectiveTeam[\s\S]*contactAllocation\?\.streamLabel[\s\S]*function renderFacilityOverviewRosterOnlyCard[\s\S]*Roster only/, "On shift should group confirmed contact-stream overrides while keeping displaced roster entries separate");
