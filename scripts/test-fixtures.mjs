...
assert.equal(isApprovedReparseOmission({ source: "VHH", title: "VHH: Clinical Support", rawValue: "Public Holiday" }), true, "a VHH public-holiday placeholder is not a clinician event");
...
assert.match(appSource, /function openFacilityOverview[\s\S]*await loadFacilityOverviewMetadata\(\);[\s\S]*const facilities = facilityOverviewFacilityOptions\(\)/, "Every At a glance tab should refresh published facility metadata before deriving its ED selector");
