...
- Production now runs zero-D1 safety commit `a146029` (or its documentation-only
  descendant). The earlier instruction to keep
  Production at `a293251` was superseded by the reviewed safety deployment.
- The additional fail-closed plan in
  [`at-a-glance-zero-d1-maintenance-plan.md`](./at-a-glance-zero-d1-maintenance-plan.md)
  is implemented: stale and current clients stop before At a glance
  authentication or access reads.
